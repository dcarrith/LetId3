...
class LetId3 extends GetId3 {
	...
	private $_logger = null;
		...
		//$this->_logger = new Logger('letid3');
		//$this->_logger->pushHandler(new StreamHandler('letid3.log', Logger::WARNING));
	}
		...
		foreach( $id3 as $key => $value ) {
			...
			if ( !is_array($value) ) {
				...
				if( $key == "data" ) {
					...
					// If the key of this iteration is an essential tag, then we want to store the value (true for strict checking)
					if ( in_array( $key, $essentialTags, true )) {
				...
				// If key is one that we care about, then let's save it (true for strict checking)
				if ( in_array( $key, $essentialTags, true )) {
